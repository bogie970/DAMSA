void GeneratePDFs_v2()
{
  TFile fInput("Neutron_full_800_1e5_POT.root", "READ");

  const Double_t dE = 5; // unit in MeV
  const Double_t Ei = 5; // unit in MeV
  const Double_t Ef = 800; // unit in MeV
  ...
  for(Int_t i = 0; i < nRuns; i++)
  {
    ...
    o_macro[i] << "/gps/pos/type Surface" << std::endl;
    o_macro[i] << "/gps/pos/shape Cylinder" << std::endl;
    o_macro[i] << "/gps/pos/centre 0. 0. -6. m" << std::endl;//center of world is 0,0,0 - center of distro is - .5*vessel(5*m) - .5(l_mod + l_dump)(.6 = .5* (1m+.2m)) 
    //o_macro[i] << "/gps/pos/radius 5.5 m" << std::endl;
//what would radius be in the context of a cylinder here.. would then legnth also be needed? ...
...
//.v2 addition for z position...what is meant by the 56th and 165th bin??



 // biasz
    // The first bin of z-histogram is 56-th bin of the histogram hz. (where z = -5.5 in meter).
    // and the last bin of z-histogram is 165-th bin.
    o_macro[i] << "/gps/hist/type biasz" << std::endl;
    for(Int_t j = 56; j <= 165; j++)
    {
      if( j == 56 ) // if we're filling in the first bin info, we should write bin lower edge. This is the rule required by Geant4.
      {
        z_real = hz[i]->GetBinLowEdge(j);
        z_gps = 0.5 + z_real/11.;
        o_macro[i] << "/gps/hist/point " << z_gps << " " << hz[i]->GetBinContent(j) << std::endl;
        continue;
      }
      else
      {
    ...
    o_macro[i] << "/tracking/verbose 0" << std::endl;
Int_t f = (Int_t)hKineticEnergy[i]->GetEntries() *10; //1e6 FOR CYC DISTRO
    // run!
    o_macro[i] << Form("/run/beamOn %d", f )<< std::endl;
  }
